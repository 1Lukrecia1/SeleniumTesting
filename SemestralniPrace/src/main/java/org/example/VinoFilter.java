...
//        inp.click();
    ...
    public void selectRatingAndRegion(){
        ...
        //System.out.println(setValues[2]);
...
//        WebDriver.Timeouts timeouts = webDriver.manage().timeouts().setScriptTimeout(20, TimeUnit.SECONDS);
    ...
    public void inputsOfRatingAndRegion() {
        regionInput.sendKeys(setValues[2]);

        if(Integer.parseInt(setValues[1]) <38) {
            ratingInput36.click();
        } else if (Integer.parseInt(setValues[1]) < 42) {
            ratingInput38.click();
        } else {
            ratingInput42.click();
        }
    ...
    private void clickTheRegion() {
        for (int i = 1; i <= 12; i++) {
            String cssSelector = String.format("#explore-page-app > div > div > div.explorerPage__columns--1TTaK > div.explorerPage__filters--1Hmly > div > div:nth-child(5) > div.filterPills__items--_grOA > label:nth-child(%d)", i);
            if (isElementPresent(cssSelector)){
                WebElement span = webDriver.findElement(new By.ByCssSelector(cssSelector));
                if (Objects.equals(span.getText(), setValues[2])){
                    span.click();
                    return;
                }
            }
    ...
    private boolean isElementPresent(String cssSelector) {
        try {
            ...
            webDriver.findElement(new By.ByCssSelector(cssSelector));
